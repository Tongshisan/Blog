...
Promise. prototype. catch 用于执行出错时的回调, 是特殊的 then 方法, . catch 之后, 可以继续 . then
...
4.  promises 中有一个被 rejected, Promise. all 的状态就变为 rejected, 此时第一个被 reject 的实例的返回值, 会传递给 Promise. all 的回调函数
